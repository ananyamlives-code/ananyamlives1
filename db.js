...
async function seedCatalogProducts() {
    ...
    if (usePg) {
        try {
            const existingCount = await pool.query('SELECT COUNT(*)::int AS count FROM products');
            ...
            for (const product of catalogProducts) {
                await pool.query(
                    `INSERT INTO products (id, title, subtitle, category, price, original_price, image_url, gallery_images, description, rating, review_count, stock, features, is_active, created_at)
                    ...
                    [
                        ...
                        product.image_url || 'images/manchatti.png',
                        JSON.stringify(Array.isArray(product.gallery_images) ? product.gallery_images : []),
                        product.description || '',
            ...
            console.log('✅ Catalog seed applied to PostgreSQL');
        } catch (error) {
            console.warn('⚠️ Catalog seed for PostgreSQL failed:', error.message);
        }
        return;
    }
    ...
    const missingProducts = catalogProducts.filter(product => !local.products.some(existing => String(existing.id) === String(product.id)));
    if (!missingProducts.length) return;

    local.products = [...missingProducts.map(product => ({
        id: product.id,
        title: product.title,
        ...
        image_url: product.image_url || 'images/manchatti.png',
        gallery_images: Array.isArray(product.gallery_images) ? product.gallery_images : [product.image_url || 'images/manchatti.png'],
    ...
    saveLocalDb(local);
    console.log(`✅ Added ${missingProducts.length} catalog products to the local fallback store.`);
}
    ...
    if (pool) {
        try {
            ...
            if (process.env.DATABASE_URL || process.env.PGHOST) {
                console.warn('🚫 Production database is configured but unavailable; not falling back silently.');
                usePg = false;
                return false;
            }
...
async function getAllProducts() {
    ...
    const fallbackDb = readLocalDb();
    const seededCatalog = Array.isArray(catalog) ? catalog : [];
    const combinedProducts = [...fallbackDb.products, ...seededCatalog.filter(product => !fallbackDb.products.some(item => String(item.id) === String(product.id)))];
    fallbackDb.products = combinedProducts;
    if (!fallbackDb.products.length) {
        saveLocalDb(fallbackDb);
    }
...
async function ensureAdminFromEnv() {
    ...
    if (existingUser) {
        if (usePg) {
            const res = await pool.query(
                'UPDATE users SET password_hash = $1, role = $2, name = COALESCE(name, $3) WHERE email = $4 RETURNING *',
                [passwordHash, 'ADMIN', 'Admin User', email]
            );
            return res.rows[0] || existingUser;
        }
