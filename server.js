...
const JWT_SECRET = process.env.JWT_SECRET || 'ananyamlives_super_secret_jwt_key_2026';
...
async function sendTransactionalEmail(toEmail, subject, htmlContent, attachments = []) {
    ...
    if (!transporter) {
        throw new Error('SMTP credentials are missing. Set EMAIL_USER and EMAIL_PASSWORD in .env and restart the server.');
    }
    ...
    if (transporter) {
        ...
        console.log(`ℹ️ EMAIL DISPATCH (Dev mode - SMTP credentials not set in .env): Target [${toEmail}] Subject [${subject}]`);
        return false;
    }
}
...
app.put('/api/admin/orders/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        updates.payment_method = 'Razorpay Online';
